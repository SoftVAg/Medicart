...
import onlineShopping from "./dashboardImages/onlineShopping.png";
...
export default function Dashboard() {
  ...
  const yearlyStatsData = useMemo(() => {
    ...
    if (statRange === "monthly") {
       console.log("Monthly line source:", analytics?.monthly?.lineCharts?.engagement);
      const src = analytics?.monthly?.lineCharts?.engagement ?? []
      return src.map(d => ({ label: d.week, value: d.activeUsers }))
    }

    const src = analytics?.weekly?.lineCharts?.weeklyStats ?? []
        ...
        <div className="flex flex-wrap gap-2 justify-around ">
          ...
          <div className="bg-[#16213E] rounded-2xl shadow-sm p-7 h-full">
            ...
            <div className="text-2x2 font-bold text-white mb-1">
              ...
              {
                  statRange === "weekly"
                  ? `${analytics?.metrics?.weeklyRevenue ?? 0} visits`
                  : statRange === "monthly"
                  ? `${analytics?.metrics?.monthlyRevenue ?? 0} visits` 
                  : `${analytics?.metrics?.yearlyRevenue} visits`
              }
          ...
          <div className="bg-[#16213E] rounded-2xl shadow-sm p-7 h-full">
            ...
            <div className="text-2x2 font-bold text-white mb-1">
                {
                  salesRange === "weekly"
                  ? `$${analytics?.metrics?.weeklyRevenue ?? 0} `
                  : salesRange === "monthly"
                  ? `$${analytics?.metrics?.monthlyRevenue ?? 0}` 
                  : `$${analytics?.metrics?.yearlyRevenue}`
              }
            </div>
